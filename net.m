...
function res = grad(model, data, wd_coefficient)
  ...
  % class. error derivative w.r.t. softmax input (given)
  input_error = class_prob - data.targets;

  hid_to_class_grad = input_error * hid_output';

  hid_activation_deriv = hid_output.*(1-hid_output);
  input_to_hid_grad = model.hid_to_class' * input_error ...
      .* hid_activation_deriv * data.inputs';

  % Average classification error gradients + weight decay
  num_elems = size(class_input,2);
  res.hid_to_class = hid_to_class_grad/num_elems + wd_coefficient * model.hid_to_class;
  res.input_to_hid = input_to_hid_grad/num_elems + wd_coefficient * model.input_to_hid;
